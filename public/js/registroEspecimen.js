...
async function ejecutarEliminacion() {
  ...
  try {
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=eliminar",
      {
        method: "POST",
        body: formData,
      },
    ...
    try {
      ...
      mostrarModal(
        "Error del Servidor: Revisa la consola (F12) para ver el mensaje oculto de PHP.",
      );
    }
  } catch (error) {
    console.error(error);
...
function abrirModalActualizar(datos) {
  ...
  if (typeof datos === "object") {
    document.dispatchEvent(
      new CustomEvent("precargarTaxonomiaActualizar", { detail: datos }),
    );
  }
}
...
function cerrarModalActualizar() {
  ...
  ["mod_familia", "mod_subfamilia", "mod_genero", "mod_especie"].forEach(
    function (id) {
...
async function fetchFotoDB(codigo, offset) {
  ...
  try {
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=obtenerImagenCarrusel",
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const map = L.map("map").setView([9.9, -83.68], 13);
  ...
  function textoEspecie(item) {
    ...
    const nombreCientifico =
      item.nombre_cientifico || item.nombreCientifico || "";
  ...
  async function cargarFamiliasPorOrden() {
    ...
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=obtenerFamiliasPorOrden&id_orden=" +
        ordenSelect.value,
    );
  ...
  async function cargarSubfamiliasPorOrden() {
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=obtenerSubfamiliasPorOrden&id_orden=" +
        ordenSelect.value,
    );
  ...
  async function cargarSubfamiliasPorFamilia() {
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=obtenerSubfamiliasPorFamilia&id_familia=" +
        familiaSelect.value,
    );
  ...
  async function cargarGenerosPorFamilia() {
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=obtenerGenerosPorFamilia&id_familia=" +
        familiaSelect.value,
    );
  ...
  async function cargarGenerosPorSubfamilia() {
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=obtenerGenerosPorSubfamilia&id_sub_familia=" +
  ...
  async function cargarEspeciesPorGenero() {
    const response = await fetch(
      "index.php?controlador=RegistroEspecimen&accion=obtenerEspeciesPorGenero&id_genero=" +
        generoSelect.value,
    );
  ...
  ordenSelect.addEventListener("change", async function () {
    ...
    if (ordenSelect.value === "SP") {
      mostrarModal(
        "Debe seleccionar al menos un orden real para registrar el espécimen.",
      );
  ...
  function llenarSelectModal(combo, datos, texto, tipo) {
    ...
    datos.forEach(function (item) {
      ...
      option.textContent =
        tipo === "especie" ? textoEspecie(item) : item.nombre;
  ...
  async function precargarModalActualizar(datos) {
    ...
    modSubfamiliaSelect.value = normalizarValorModal(
      datos.id_subfamilia,
      false,
    );
    ...
    modOrdenSelect.addEventListener("change", async function () {
      ...
      if (modOrdenSelect.value === "" || modOrdenSelect.value === "SP") {
        if (modOrdenSelect.value === "SP") {
          mostrarModal(
            "Debe seleccionar un orden real para actualizar el especimen.",
          );
        }
        return;
      }
  ...
  document
    .getElementById("formRegistroEspecimen")
  ...
  const formActualizarTaxonomia = document.getElementById(
    "formActualizarTaxonomia",
  );
  if (formActualizarTaxonomia) {
    formActualizarTaxonomia.addEventListener("submit", async function (e) {
      ...
      const formData = new FormData(this);
      const response = await fetch(
        "index.php?controlador=RegistroEspecimen&accion=actualizarTaxonomia",
        {
          method: "POST",
          body: formData,
        },
  ...
  if (formAgregarFotos) {
    formAgregarFotos.addEventListener("submit", async function (e) {
      ...
      const formData = new FormData(this);
      const response = await fetch(
        "index.php?controlador=RegistroEspecimen&accion=agregarImagenes",
        { method: "POST", body: formData },
      );
      const data = await response.json();
      if (data.success) {
  ...
  const todasLasGavetas = Array.from(gavetaSelect.options).filter(
    (op) => op.value !== "",
  );
  ...
  const todosLosViales = Array.from(vialSelect.options).filter(
    (op) => op.value !== "",
  );
...
// HU-17: Plantas
// =====================
let codigoPlantasActual = "";

function abrirModalPlantas(codigo) {
  codigoPlantasActual = codigo;
  document.getElementById("txtCodigoPlantas").textContent = codigo;
  document.getElementById("modalPlantas").style.display = "flex";
  cargarPlantasEspecimen(codigo);
  cargarTodasLasPlantas();
}

function cerrarModalPlantas() {
  document.getElementById("modalPlantas").style.display = "none";
  codigoPlantasActual = "";
}

function cargarPlantasEspecimen(codigo) {
  fetch(
    "?controlador=RegistroEspecimen&accion=listarPlantasEspecimen&codigo=" +
      codigo,
  )
    .then((r) => r.json())
    .then((plantas) => {
      const div = document.getElementById("listaPlantas");
      if (plantas.length === 0) {
        div.innerHTML = '<p style="color:#999;">No hay plantas asociadas.</p>';
        return;
      }
      let html = "";
      plantas.forEach((p) => {
        html += `<div style="display:flex; justify-content:space-between; align-items:center; padding:6px; border-bottom:1px solid #eee;">
                    <span><strong>${p.nombre_comun}</strong> <em style="color:#666;">(${p.nombre_cientifico})</em></span>
                    <button type="button" style="background:#e74c3c; padding:4px 10px; margin:0; font-size:12px;"
                        onclick="quitarPlantaEspecimen('${p.id}')">Quitar</button>
                </div>`;
      });
      div.innerHTML = html;
    });
...
function cargarTodasLasPlantas() {
  fetch("?controlador=RegistroEspecimen&accion=listarTodasLasPlantas")
    .then((r) => r.json())
    .then((plantas) => {
      const sel = document.getElementById("selectNuevaPlanta");
      sel.innerHTML = '<option value="">Seleccione una planta</option>';
      plantas.forEach((p) => {
        sel.innerHTML += `<option value="${p.id}">${p.nombre_comun} (${p.nombre_cientifico})</option>`;
      });
    });
}

function agregarPlantaEspecimen() {
  const idPlanta = document.getElementById("selectNuevaPlanta").value;
  if (!idPlanta) {
    mostrarMensajePlantas("Debe seleccionar una planta", "error");
    return;
  }
  const formData = new FormData();
  formData.append("codigo", codigoPlantasActual);
  formData.append("id_planta", idPlanta);

  fetch("?controlador=RegistroEspecimen&accion=asociarPlanta", {
    method: "POST",
    body: formData,
  })
    .then((r) => r.json())
    .then((data) => {
      mostrarMensajePlantas(data.mensaje, data.ok ? "exito" : "error");
      if (data.ok) cargarPlantasEspecimen(codigoPlantasActual);
    });
}

function quitarPlantaEspecimen(idPlanta) {
  fetch(
    `?controlador=RegistroEspecimen&accion=eliminarPlantaEspecimen&codigo=${codigoPlantasActual}&id_planta=${idPlanta}`,
  )
    .then((r) => r.json())
    .then((data) => {
      mostrarMensajePlantas(data.mensaje, data.ok ? "exito" : "error");
...
function mostrarMensajePlantas(msg, tipo) {
  const div = document.getElementById("mensajePlantas");
  div.style.color = tipo === "exito" ? "#28a745" : "#e74c3c";
  div.textContent = msg;
